          ...
          <section className="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200">
            <div className={`grid gap-0 bg-slate-950 ${images.length > 1 ? 'lg:grid-cols-[96px_1fr]' : ''}`}>
              ...
              <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                ...
                {isPremium && <span className="inline-flex items-center gap-1 rounded-md bg-amber-400 px-3 py-1 text-xs font-black text-amber-950 shadow-sm"><Crown size={14} /> Premium</span>}
              </div>
          ...
          <aside className="space-y-4">
            <section className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 md:p-7">
              ...
              <div className="mt-5 grid gap-3 text-sm text-slate-600 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                <div className="flex items-center gap-2 rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200"><MapPin size={17} /> {listing.location || 'Konum yok'}</div>
                <div className="flex items-center gap-2 rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200"><Eye size={17} /> {listing.view_count || 0} görüntülenme</div>
                <div className="flex items-center gap-2 rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200"><CalendarDays size={17} /> {formatDate(listing.created_at)}</div>
                <div className="flex items-center gap-2 rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200"><ShieldCheck size={17} /> Admin onaylı</div>
              </div>
            </section>
            ...
            <section className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 md:p-6">
              ...
              <div className="grid gap-3 text-sm">
                ...
                <div className="flex items-center justify-between rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200"><span className="flex items-center gap-2 text-slate-500"><CheckCircle2 size={16}/> Durum</span><strong>{listing.condition || listing.metadata?.condition || 'Belirtilmedi'}</strong></div>
                <div className="flex items-center justify-between rounded-lg bg-slate-50 p-3 ring-1 ring-slate-200"><span className="flex items-center gap-2 text-slate-500"><ShieldCheck size={16}/> Güven</span><strong>Admin onaylı</strong></div>
              </div>
            </section>
            ...
            <section className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 md:p-7">
              ...
              {whatsappPhone && (
                <a href={`https://wa.me/${whatsappPhone}`} target="_blank" rel="noreferrer" className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-cyan-700 px-4 py-3 text-sm font-black text-white hover:bg-cyan-800">
                  <MessageCircle size={17} /> WhatsApp ile yaz
                </a>
              )}

              {email && (
                <a href={`mailto:${email}?subject=NouMarket ilanı: ${encodeURIComponent(listing.title)}`} className="mt-3 flex w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-black text-slate-800 hover:bg-slate-100">

...
import { CATEGORY_TREE, FIELD_DEFINITIONS } from '@/lib/categorySchema';

const NEW_CALEDONIA_LOCATIONS = {
  'Nouméa': ['Centre-ville', 'Anse Vata', 'Baie des Citrons', 'Orphelinat', 'Receiving', 'Motor Pool', 'Trianon', 'Faubourg Blanchot', 'Vallée des Colons', 'Magenta', 'Tina', 'Ducos', 'Rivière Salée', 'Kaméré', 'Montravel', 'Portes de Fer'],
  'Dumbéa': ['Koutio', 'Auteuil', 'Dumbéa-sur-Mer', 'Nakutakoin', 'Katiramona', 'Plaine Adam', 'Val Suzon'],
  'Mont-Dore': ['Boulari', 'Robinson', 'Yahoué', 'Pont-des-Français', 'Saint-Michel', 'La Coulée', 'Plum', 'Mouirange', 'Prony'],
  'Païta': ['Centre', 'Savannah', 'Val Boisé', 'Ondémia', 'Tontouta', 'Karikaté', 'Gadji'],
  'Bourail': ['Centre', 'Roche Percée', 'Poé', 'Nessadiou', 'Gouaro'],
  'Koné': ['Centre', 'Foué', 'Baco', 'Tiaoué'],
  'La Foa': ['Centre', 'Farino', 'Moindou', 'Sarraméa'],
  'Pouembout': ['Centre', 'Tipenga'],
  'Koumac': ['Centre', 'Néhoué'],
  'Poya': ['Centre', 'Népoui'],
  'Canala': ['Centre', 'Nakety'],
  'Thio': ['Centre', 'Thio Mission'],
  'Yaté': ['Centre', 'Touaourou', 'Goro'],
  'Lifou': ['Wé', 'Xépénéhé', 'Chépénéhé'],
  'Maré': ['Tadine', 'La Roche', 'Wabao'],
  'Île des Pins': ['Vao', 'Kuto', 'Oro']
};

const CITY_OPTIONS = Object.keys(NEW_CALEDONIA_LOCATIONS);
...
export default function ListingForm({ onClose, onCreate, user, profile }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/50 p-2 backdrop-blur-sm md:p-5">
      <div className="mx-auto flex max-h-[96vh] max-w-6xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        <form onSubmit={submit} className="grid min-h-0 flex-1 overflow-hidden lg:grid-cols-[340px_minmax(0,1fr)]">
          <aside className="min-w-0 overflow-y-auto overflow-x-hidden border-b border-slate-200 bg-slate-50 p-3 lg:border-b-0 lg:border-r">

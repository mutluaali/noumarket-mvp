...
export function calculateProfileTrust(profile = {}, user = {}) {
  const checks = [
    ...
    { key: 'verified', label: 'Moderasyon doğrulaması', ok: Boolean(profile?.is_verified), points: 10 },
  ];
...
export function calculatePublicSellerTrust(profile = {}) {
  return calculateProfileTrust(profile, { email: true });
}

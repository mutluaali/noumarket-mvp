...
export function withTimeoutFallback(promise, timeoutMs = 8000, fallbackData = null, onTimeout) {
  let timer;
  ...
  return Promise.race([promise, timeout])
    .then((value) => (value === undefined ? fallbackData : value))
    .catch((error) => {
      console.warn('withTimeoutFallback:', getErrorMessage(error));
      return fallbackData;
    })

...
function withRequestTimeout(promise, ms = 6500, label = 'MESSAGES_TIMEOUT') {
  let timer;
  return Promise.race([
    promise,
...
export async function getOrCreateConversation({ listingId, buyerId, sellerId }) {
  ...
  await assertUserNotSuspended(supabase, buyerId);

  if (buyerId === sellerId) {
    throw new Error('Kendi ilanın için mesajlaşma başlatamazsın.');
  }

  const { data: existing, error: existingError } = await withRequestTimeout(
    supabase
      .from('conversations')
      ...
      .maybeSingle(),
    6500,
    'Mesajlasma baslatma zaman asimina ugradi.'
  );
...
export async function sendMessage({ conversationId, senderId, body }) {
  ...
  await assertUserNotSuspended(supabase, senderId);
  ...
  const { data, error } = await withRequestTimeout(
    supabase
      ...
      .single(),
    6500,
    'Mesaj gonderme zaman asimina ugradi.'
  );
...
export async function getMyConversations(userId) {
  ...
  const withTimeout = (promise, ms = 12000) => {
    ...
    return Promise.race([
      promise,
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error('MESSAGES_TIMEOUT')), ms);
      }),
    ]).finally(() => clearTimeout(timer));
  };

...
const PUBLIC_SENSITIVE_LISTING_KEYS = ['seller_email', 'sellerEmail', 'email'];

/** Strip seller email from public listing payloads (HTML, API browse, permalink). */
export function sanitizePublicListing(row) {
  if (!row || typeof row !== 'object') return row;

  const sanitized = { ...row };
  for (const key of PUBLIC_SENSITIVE_LISTING_KEYS) {
  ...
  if (sanitized.profiles && typeof sanitized.profiles === 'object') {
...
export function sanitizePublicListingList(rows) {
  return Array.isArray(rows) ? rows.map(sanitizePublicListing) : [];
}

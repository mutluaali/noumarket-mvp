...
import { analyzeListingSafety } from './moderation';
...
export async function getApprovedListings(filters = {}) {
  const payload = await fetchJson(buildListingsQuery(filters), { timeoutMs: 9000 });
  return payload.data || [];
}

export async function getAdminListings() {
  const payload = await fetchJson('/api/admin/listings', { timeoutMs: 9000 });
  return payload.data || [];
}
...
export async function createListing(payload) {
  ...
  const safety = analyzeListingSafety(payload);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      metadata: {
        ...
        moderation_risk_score: safety.riskScore,
        moderation_risk_level: safety.level,
        moderation_risk_reasons: safety.reasons,
        moderation_status: safety.shouldHold ? 'review' : 'pending',
      },
      status: 'pending',
      is_featured: false,
      featured_until: null,
